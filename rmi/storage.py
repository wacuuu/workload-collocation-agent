...
def convert_to_prometheus_exposition_format(metrics: List[Metric]) -> str:
    ...
    for metric in metrics:
        ...
        if metric.type is not None:
            if metric.type != MetricType.GAUGE and metric.type != MetricType.COUNTER:
                log.log(logger.TRACE, 'Wrong or not supported metric type')
            else:
...
class KafkaStorage(Storage):
    ...
    def callback_on_delivery(self, err, msg) -> None:
        ...
        if err is not None:
            ...
            log.log(logger.ERROR,
                    'kafka storage failed to send message; error message: {}'
                    .format(err))
        else:
            log.log(logger.TRACE,
                    'kafka storage succeeded to send message; message: {}'
                    .format(msg))
    ...
    def store(self, metrics: List[Metric]) -> None:
        ...
        if self.error_from_callback is not None:
            ...
            # befere reseting self.error_from_callback we

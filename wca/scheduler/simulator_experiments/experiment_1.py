...
task_definitions = [
    ...
    Task(name='cpu', requested=Resources({rt.CPU: 10, rt.MEM: 50, rt.MEMBW: 1, rt.WSS: 1})),
    Task(name='mem', requested=Resources({rt.CPU: 1, rt.MEM: 100, rt.MEMBW: 1, rt.WSS: 1})),
    Task(name='mbw', requested=Resources({rt.CPU: 1, rt.MEM: 1, rt.MEMBW: 10, rt.WSS: 1})),
]
...
def run():
    ...
    experiments_set__generic(
        ...
        (30*SCALE,),
        (
            (TaskGenerator_equal, dict(task_definitions=task_definitions, replicas=10*SCALE)),
            # (TaskGenerator_equal, dict(task_definitions=task_definitions, replicas=15)),
            (TaskGenerator_classes, dict(task_definitions=task_definitions, counts=dict(mbw=3*SCALE, cpu=12*SCALE, mem=15*SCALE))),
            (TaskGenerator_classes, dict(task_definitions=task_definitions, counts=dict(mbw=20*SCALE, cpu=5*SCALE, mem=5*SCALE))),
            (TaskGenerator_classes, dict(task_definitions=task_definitions, counts=dict(mbw=30*SCALE, cpu=0*SCALE, mem=0*SCALE))),
            (TaskGenerator_classes, dict(task_definitions=task_definitions, counts=dict(mbw=0*SCALE, cpu=0*SCALE, mem=30*SCALE))),
        ...
        (
            prepare_nodes(dict(
                ...
                dict(aep=0, dram=6*SCALE),
                nodes_dimensions,
            ),
            prepare_nodes(dict(
                aep={rt.CPU: 40, rt.MEM: 1000, rt.MEMBW: 40, rt.MEMBW_READ: 40, rt.MEMBW_WRITE: 10},
                dram={rt.CPU: 40, rt.MEM: 192, rt.MEMBW: 200, rt.MEMBW_READ: 150, rt.MEMBW_WRITE: 150}, ),
                dict(aep=2*SCALE, dram=6*SCALE),
                nodes_dimensions,
            ),
            prepare_nodes(dict(
                aep={rt.CPU: 40, rt.MEM: 1000, rt.MEMBW: 40, rt.MEMBW_READ: 40, rt.MEMBW_WRITE: 10},
                dram={rt.CPU: 40, rt.MEM: 192, rt.MEMBW: 200, rt.MEMBW_READ: 150, rt.MEMBW_WRITE: 150}, ),
                dict(aep=0, dram=8*SCALE),
                nodes_dimensions,
            ),
            # prepare_nodes(dict(

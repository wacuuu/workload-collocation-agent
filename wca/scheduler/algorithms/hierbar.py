...
class HierBAR(BAR):
    ...
    def app_fit_nodes(self, node_names, app_name, data_provider_queried
        ...
        for class_shape, node_names_of_this_shape in shapes_to_nodes.items():
            ...
            sum_resouces_of_nodes = reduce(sum_resources, nodes_of_this_shape)
            averaged_resources_of_class = divide_resources(
                sum_resouces_of_nodes,
                {r:len(nodes_of_this_shape) for r in sum_resouces_of_nodes.keys()},
            )
        ...
        for class_shape, class_bar_variance in sorted(class_bar_variances.items(), key=lambda x:x[1]):
            ...
            accepted_node_names = set(node_names) & set(best_node_names_according_shape)
        ...
        for failed_node_name in failed_names:
            failed[failed_node_name] = 'Not best class!'

        log.debug(
            '[Prioritize][app=%s] nodes=[%s] best_class=%r best_class_variance=%.3f best_nodes=[%s]',
